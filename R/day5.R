...
parse_data <- function(file){
  tibble(vents = readLines(file)) |> 
    ...
    mutate(vent = as.factor(row_number()),
...
expand_path <- function(df) {
  expand_grid(x = df$x1:df$x2,  y=df$y1:df$y2)
}

horizontal_and_vertical_paths <- function(df){
  df |> 
    filter(slope %in% c(0, -Inf, Inf)) |> 
    select(-slope) |> 
    group_by(vent) |> 
    nest(vector = c(x1,x2,y1,y2)) |> 
    mutate(points = map(vector, expand_path))
}

####

df <- parse_data('data/day5.txt')
# plot_diagram(df)

paths <- horizontal_and_vertical_paths(df)
