...
# use the median position and sum distance
cost <- input |>
  as_tibble() |>
  setNames('pos') |>
  mutate(cost = abs(pos - median(input))) |>
  pull(cost) |>
  sum()

cat('Crab submarine horizontal destination: ', median(input))
cat('Crab submarine fuel cost: ', cost)
#
# --- Part Two ---
...
get_cost <- function(dest) {
  plan <-
    input |>
    as_tibble() |>
    setNames('pos') |>
    mutate(moves = abs(pos - round(dest))) |>
    mutate(cost = (moves + 1) * moves / 2)
  plan |> pull(cost) |> sum()
}
...
min(
  get_cost(round(minima$minimum - 1)),
  get_cost(round(minima$minimum)),
  get_cost(round(minima$minimum + 1))
)
